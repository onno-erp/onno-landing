...
const HERO_IMAGE = '/images/hero-night.png'
...
export function Hero() {
  ...
  return (
    <section
      className="overscroll-top-edge relative flex min-h-[100svh] flex-col overflow-hidden bg-[#06152f] bg-cover bg-center"
    ...
    >
      ...
      <main className="relative z-20 mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-8 px-5 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(420px,0.9fr)] lg:gap-12 lg:px-10">
        <div className="relative z-10 flex flex-col items-start text-left">
          <h1 className="text-[40px] font-normal leading-[1.02] tracking-tight text-white min-[400px]:text-[44px] sm:text-6xl lg:text-[64px] xl:text-[76px]">
          ...
          <p className="animate-fade-up mt-5 max-w-xl text-base leading-relaxed text-blue-100/70 [animation-delay:240ms] sm:mt-6 sm:text-lg lg:text-xl">
            {copy.hero.description}
          </p>

          <div className="animate-fade-up mt-6 flex flex-wrap items-center justify-start gap-3 [animation-delay:340ms]">
            <a href={telegramLink} target="_blank" rel="noreferrer" className="telegram-cta inline-flex items-center gap-2 overflow-hidden rounded-full bg-white px-6 py-3 text-sm font-medium text-[#07162f] transition-[background-color,box-shadow] hover:bg-blue-50 hover:shadow-lg hover:shadow-black/20">
              {copy.hero.primary} <AnimatedTelegramIcon />
            </a>
            <a href="https://github.com/onno-erp/onno-framework" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-medium text-white/80 ring-1 ring-white/25 transition-colors hover:bg-white/10 hover:text-white">
      ...
      <img src={GRASS_IMAGE} alt="" className="pointer-events-none absolute bottom-0 left-0 z-10 w-full select-none brightness-[0.28] saturate-[0.7] hue-rotate-[145deg]" />
    </section>
  )
}
